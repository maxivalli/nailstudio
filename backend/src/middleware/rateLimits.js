...
export const getAllAppointments = async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT id, name, whatsapp, appointment_date, appointment_hour,
              status, service_name, service_price, reminder_sent, created_at
       FROM appointments ORDER BY appointment_date, appointment_hour`
    );
    res.json({ success: true, data: result.rows });
  } catch (err) {
...
export const getClientHistory = async (req, res) => {
  ...
  // Exigir exactamente 10 dígitos para evitar falsos positivos
  const searchDigits = cleaned.slice(-10);
  if (searchDigits.length < 10) {
    return res.status(400).json({ success: false, error: 'Ingresá un número de 10 dígitos, ej: 3408123456.' });
  }
  try {
    const result = await pool.query(`
      SELECT
